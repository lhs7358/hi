...
while True:
    ...
    for idx, (x, y, w, h) in enumerate(faces_sorted_by_size, 1):
        ...
        # Add an extra dimension to the ROI for input to the emotion detection model
        roi_input = np.expand_dims(roi_normalized, axis=0)
        roi_input = np.expand_dims(roi_input, axis=-1)

        # Use the emotion detection model to predict the probability of each emotion label for the ROI
        emotion_probabilities = model.predict(roi_input)[0]

        # Select the emotion label with the highest predicted probability as the dominant emotion for the face
